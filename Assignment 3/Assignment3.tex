...
\begin{align}
   \qquad \int\limits_{-\infty}^{\infty}p\brak{x}dx &=\int\limits_{-\infty}^{1}p\brak{x}dx + \int\limits_{1}^{\infty}p\brak{x}dx   \\\nonumber\\
 &= 0 + \int\limits_{1}^{\infty}\dfrac{A}{x^\mu}dx \:\:\, \brak{\because p\brak{x} = 0 \;\text{in} \brak{-\infty,1}}\\\nonumber\\
  &= \left[\frac{-A}{\brak{\mu-1}x^{\mu-1}}\right]_{1}^{\infty}  \quad \brak{\because \mu > 1}\\\nonumber\\
